...
public class OrangeEgg : MonoBehaviour
{
    ...
    void Update()
    {
        // Handle pause/unpause for orange eggs
    ...
    {
        if (other.gameObject.CompareTag("Player")) // Pallet catches orange egg
        {
            // Trigger pallet freeze effect!
            if (EggGameManager.Instance != null)
            {
                EggGameManager.Instance.TriggerPalletFreeze();
            }
            
            // Play freeze sound with modified pitch
            if (_audioSource != null)
            {
                _audioSource.pitch = 1.3f; // Higher pitch for freeze effect
                _audioSource.Play();
            }
            
            // Destroy the orange egg
            Destroy(gameObject);
        }
        else if (other.gameObject.CompareTag("Ground")) // Orange egg hits ground (missed)
        {
            // Play miss sound
            if (_audioSource != null)
            {
    ...
    {
        // Handle slope collisions - orange eggs should bounce/roll naturally
