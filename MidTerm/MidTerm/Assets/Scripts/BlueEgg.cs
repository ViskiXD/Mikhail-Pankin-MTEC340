...
public class BlueEgg : MonoBehaviour
{
    ...
    void Update()
    {
        // Handle pause/unpause for blue eggs
    ...
    {
        if (other.gameObject.CompareTag("Player")) // Pallet catches blue egg
        {
            // Trigger slow motion effect!
            if (EggGameManager.Instance != null)
            {
                EggGameManager.Instance.TriggerSlowMotion();
            }
            
            // Play catch sound with modified pitch
            if (_audioSource != null)
            {
                _audioSource.pitch = 0.7f; // Lower pitch for blue egg effect
                _audioSource.Play();
            }
            
            // Destroy the blue egg
            Destroy(gameObject);
        }
        else if (other.gameObject.CompareTag("Ground")) // Blue egg hits ground (missed)
        {
            // Play miss sound
            if (_audioSource != null)
            {
    ...
    {
        // Handle slope collisions - blue eggs should bounce/roll naturally
